...
{
    [TestFixture]
    public class SubmitCommandUpdateTests
    {

        [Test]
        public async Task TestSubmitUpdateSubmissionFailure()
        {
            ...
            A.CallTo(() => client.UpdateSubmission("myapp", "submissionid", JObject.Parse(updatedJson).ToString())).Returns(Task.Factory.StartNew(() => false));
            ...
            result.Should().BeNegative("the update of the submission failed");
            logger.Message.Last().Should().Be(stringprovider.GetString(Strings.SubmitUpdateError), "the submission failed to update");
            ...
            A.CallTo(() => client.Commit("myapp", "submissionid")).MustNotHaveHappened();
        }



        [Test]
        public async Task TestSubmitUpdateFlightFailure()
        {
            var client = A.Fake<IStoreClient>();
            A.CallTo(() => client.Login("myId", "myKey", "myTenantId")).Returns(Task.Factory.StartNew(() => true));
            ...
            A.CallTo(() => client.UpdateSubmission("myapp", "myflight", "submissionid", JObject.Parse(updatedJson).ToString())).Returns(Task.Factory.StartNew(() => false));
            var logger = new MockLogger();
            var stringprovider = new StringProvider();
            var command = new SubmitCommand(client, logger, stringprovider);
            var packageFileLocation = $"{this.GetExecutingPath()}\\Files\\package.zip";
            ...
            A.CallTo(() => client.Commit("myapp", "myflight", "submissionid")).MustNotHaveHappened();
        }
    }
}
